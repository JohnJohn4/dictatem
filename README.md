...
Run this in PowerShell. It installs [`uv`](https://docs.astral.sh/uv/) if needed, auto-detects whether you have an NVIDIA GPU (picking the CUDA or CPU-lean dependency set accordingly), installs Dictatem **pinned to the v0.2.2 release**, and launches it to the system tray:

```powershell
irm https://raw.githubusercontent.com/JohnJohn4/dictatem/v0.2.2/install.ps1 | iex
```

Piping a script from the internet into `iex` runs it immediately. If you'd rather read it first, open [that URL](https://raw.githubusercontent.com/JohnJohn4/dictatem/v0.2.2/install.ps1) in your browser and run it once you're satisfied.
...
**Updating.** Re-run the one-liner with a newer version tag in the URL (e.g. `.../v0.2.2/install.ps1`); see the [latest release](https://github.com/JohnJohn4/dictatem/releases/latest) for the current tag.
