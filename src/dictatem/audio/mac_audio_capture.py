...
class MacAudioCapture:
    ...
    One engine is created lazily on the first ``start()`` and reused; each
    ``start()`` installs an input tap and starts the engine, each
    ``stop()``/``close()`` removes the tap and stops the engine — so the mic is
    released *between* dictations (the privacy win the sounddevice keep-open
    reserve would have lost), and a later ``start()`` re-arms cleanly.
    AVAudioEngine ``stop()`` is bounded, not unbounded like ``Pa_StopStream``
    under load, so it is safe on the Qt main thread.
    """
    ...
    def __init__(self, config: Config, buffer: AudioBuffer) -> None:
        ...
        self._buffer = buffer
        # Engine + input node are created lazily on the first start() (and reused
        # thereafter) rather than here, so a Mac with no usable input device fails
        # through start()'s AudioCaptureError path — the daemon's graceful mic
        # guidance + recover-to-idle — instead of crashing daemon launch. Mirrors
        # SoundDeviceCapture, which defers all device access to start().
        ...
        # config.audio.device pins a specific input on the sounddevice backend;
        # the native backend records from the system default input node. Warn
        # rather than silently ignore, so a user who set a device knows it isn't
        # honored on macOS yet (a follow-up — see ADR-0027).
        device = config.audio.device
        if device is not None and device != "default":
            logger.warning(
                "config audio.device=%r is not yet honored by the native macOS "
                "capture backend — recording from the system default input "
                "(remove the setting to silence this).",
                device,
            )

    def start(self) -> None:
        # Drop any tap and any buffered audio left by a prior dictation before
        # re-arming: removeTapOnBus_ does not guarantee the realtime callback has
        # finished, so a late append could otherwise (a) collide with a fresh
        # installTapOnBus_ — a double-tap raises — or (b) leak the previous
        # dictation's tail into this one. Flushing here makes each dictation start
        # from an empty buffer regardless.
        self._teardown_tap()
        self._buffer.flush()
        ...
        try:
            ...
            # differ between dictations (44.1 vs 48 kHz on one machine, #161 §2),
            # so both the tap format and the resampler are rebuilt each start().
            native_fmt = self._input.outputFormatForBus_(0)  # type: ignore[union-attr]
            src_rate = float(native_fmt.sampleRate())
        except Exception as exc:
            raise AudioCaptureError(
                f"Could not open the macOS microphone input: {exc!r}"
            ) from exc
        if src_rate <= 0:
            # AVAudioEngine reports a 0 Hz input format when there is no usable
            # input — no device, or Microphone (TCC) access not granted. Surface
            # it as the error class the daemon already handles gracefully rather
            # than letting a downstream ZeroDivisionError escape uncaught.
            raise AudioCaptureError(
                "No usable microphone input (0 Hz) — grant Microphone access or "
        ...
        self._tap_block = tap
        self._input.installTapOnBus_bufferSize_format_block_(  # type: ignore[union-attr]
    ...
    def _ensure_engine(self) -> object:
        # Create the engine + input node once, lazily, and reuse across
        # dictations (the spike proved reuse is clean). Lazy so any hardware
        # failure surfaces on the start() error path, not at daemon construction.
