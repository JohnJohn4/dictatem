...
the shim execs into — not as the bundle — so ``LSUIElement`` never applies. We
make the *running process* a menu-bar accessory instead, via the Carbon
``TransformProcessType`` (a plain ``setActivationPolicy_`` removed the Dock icon
but did not register the status bar under a LaunchServices launch). None of this
fixes the TCC identity label, which still needs a signed bundle (#91).
"""
