...
def render_info_plist(
    ...
    *version* (the installed package version) is cosmetic — it surfaces in
    Finder's Get Info so a user or QA session can tell which build the shell
    was generated by. TCC identity never depends on it. ``None`` (version
    unknown) simply omits the version keys.
    """
